...
private:
	...
	public:
		...
		std::span<FreelistProxy> make_proxies(void* raw_buffer)
		{
			...
			constexpr std::size_t total_stride_count = [] (const auto& metadata_ref) {
				std::size_t sum = 0;
			...
			const auto fill_proxies = []<std::size_t... Is>(
				std::index_sequence<Is...>,
...
private:
	...
	{
		constexpr std::size_t count = Set::create_allocator_config().total_stride_count;
		constexpr std::size_t bytes = sizeof(FreelistProxy) * count;
		constexpr std::size_t buffer_bytes = alignof(FreelistProxy) + bytes;

		thread_local static std::array<std::byte, buffer_bytes> buffer {};

		void* raw_ptr = static_cast<void*>(buffer.data());
		std::size_t space = buffer_bytes;
