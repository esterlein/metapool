...
#ifndef MTP_VERBOSE_ASSERT
	#ifdef NDEBUG
		#define MTP_VERBOSE_ASSERT 0
	#else
		#define MTP_VERBOSE_ASSERT 1
	#endif
#endif
...
namespace {
	...
	static constexpr std::size_t frame_width    = 60;
	static constexpr std::size_t frame_padding  = 4;
	static constexpr std::size_t buf_size       = 64;
	static constexpr std::size_t text_max_width = frame_width - frame_padding;
}
...
{
	std::size_t len1 = std::strlen(line1);
	std::size_t len2 = line2 ? std::strlen(line2) : 0;
	std::size_t len3 = context ? std::strlen(context) : 0;

	std::size_t inner_width = std::max({len1, len2, len3});
	if (inner_width > text_max_width)
		inner_width = text_max_width;

	std::size_t width = dynamic_width ? inner_width + frame_padding : frame_width;
	...
	auto print = [width](const char* str) {
		...
		std::size_t content_width = len + 2;
		std::size_t pad_total = width - 2 - content_width;
		std::size_t pad_left = pad_total / 2;
		std::size_t pad_right = pad_total - pad_left;
...
[[noreturn]] inline void fatal(const msg& message, const char* context = nullptr)
{
	...
	if (const char* newline = std::strchr(message.text, '\n')) {
		std::size_t len1 = newline - message.text;
...
[[noreturn]] inline void fatal(const char* message, const char* context)
{
	...
	if (const char* newline = std::strchr(message, '\n')) {
		std::size_t len1 = newline - message;
