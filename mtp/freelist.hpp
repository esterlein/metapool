...
public:
	...
	void initialize(std::byte* memory, proxy_index_t proxy_index)
	{
		...
		const std::size_t total_bytes = static_cast<std::size_t>(BlockCount) * Stride;
		...
		for (std::size_t i = 0; i < static_cast<std::size_t>(BlockCount); ++i) {
	...
	inline void reset() noexcept
	{
		...
		for (std::size_t i = 1; i < static_cast<std::size_t>(BlockCount); ++i, ptr += Stride) {
