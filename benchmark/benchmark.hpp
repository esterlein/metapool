...
static constexpr std::size_t k_qword = 8;


inline std::size_t align_down8(std::size_t x)
{
	return x & ~std::size_t(7);
}


inline void touch_prefix(void* buffer, std::size_t size_bytes)
{
	std::byte* base = static_cast<std::byte*>(buffer);
	const std::size_t write_limit = align_down8(size_bytes);

	for (std::size_t offset = 0; offset < write_limit; offset += k_qword) {
		*reinterpret_cast<std::uint64_t*>(base + offset) =
			(static_cast<std::uint64_t>(offset) ^ 0x9E3779B97F4A7C15ULL);
	}
}


inline void touch_step(void* buffer, std::size_t size_bytes, std::size_t step_bytes, std::size_t bytes_limit)
{
	std::byte* base = static_cast<std::byte*>(buffer);
	const std::size_t step = (step_bytes ? step_bytes : 64);
	std::size_t bytes_written = 0;

	for (std::size_t offset = 0; offset < size_bytes && bytes_written < bytes_limit; offset += step) {
		const std::size_t aligned = align_down8(offset);
		*reinterpret_cast<std::uint64_t*>(base + aligned) = (static_cast<std::uint64_t>(offset) * 0x9E37U);
		bytes_written += k_qword;
	}
}


inline void touch_scatter(void* buffer, std::size_t size_bytes, std::size_t qword_count, std::uint32_t seed)
{
	std::byte* base = static_cast<std::byte*>(buffer);
	...
	for (std::size_t i = 0; i < qword_count; ++i) {
		const std::size_t offset = align_down8(static_cast<std::size_t>(next() % size_bytes));
		*reinterpret_cast<std::uint64_t*>(base + offset) =
			(next() ^ (static_cast<std::uint64_t>(i) << 32));
	}
}
