...
public:
	...
	{
		std::array<std::size_t, raw_test_count> sizes = {
...
private:
	...
	static constexpr std::size_t count  = 5000;
	static constexpr std::size_t frames = 100;

	static constexpr std::size_t alignment      = 8;
	static constexpr std::size_t max_pointers   = 10000;
	static constexpr std::size_t raw_test_count = 10;
	...
	static constexpr std::size_t trash_size = 4 * 1024 * 1024;
	std::vector<std::uint8_t> m_trash;
	...
	{
		std::uint64_t sum = 0;
		for (std::size_t i = 0; i < trash_size; i += 64) {
			sum += m_trash[i];
		}
	...
	void print_deltas(const std::array<T*, N>& ptrs, std::size_t count, const char* label)
	{
	...
	void print_summary_group(std::string_view label, std::span<const BenchmarkEntry> entries)
	{
		...
		std::vector<Row> rows;
		std::size_t col0 = label.length();
		std::size_t col1 = std::strlen("std (ms)");
		std::size_t col2 = std::strlen("pmr (ms)");
		std::size_t col3 = std::strlen("mtp (ms)");
		std::size_t col4 = std::strlen("mtp vs std");
		std::size_t col5 = std::strlen("mtp vs pmr");
		...
		constexpr int gap = 4;
		auto pad = [](std::size_t w) { return static_cast<int>(w + gap); };
	...
	std::array<double, 2> run_raw_alloc(std::string_view label, std::size_t size, std::size_t count, uint32_t frames)
	{
		std::array<double, 2> time;

		time[0] = run_raw_loop("malloc", size, count, frames,
			[](std::size_t s) { return std::malloc(s); },
			[](void* p, std::size_t) { std::free(p); }
		);

		auto& metapool = mtp::get_allocator<Set>();
		time[1] = run_raw_loop("metapool", size, count, frames,
			[&metapool](std::size_t s) { return metapool.alloc(s, alignment); },
			[&metapool](void* p, std::size_t) { metapool.free(static_cast<std::byte*>(p)); }
		);
	...
	void print_summary_raw(const std::array<std::size_t, raw_test_count>& sizes)
	{
		auto ratio_str = [](double base, double val) -> std::string {
			if (base == 0.0) return "-";
			double r = val >= base ? val / base : base / val;
		...
		std::vector<Row> rows;
		std::size_t col0 = std::strlen("--- raw");
		std::size_t col1 = std::strlen("malloc (ms)");
		std::size_t col2 = std::strlen("mtp (ms)");
		std::size_t col3 = std::strlen("mtp vs malloc");

		for (std::size_t i = 0; i < raw_test_count; ++i) {
	...
	template <typename T>
	double run_construct_std(std::size_t count, uint32_t frames)
	{
	...
	template <typename T>
	double run_construct_pmr(std::size_t count, uint32_t frames)
	{
	...
	template <typename T>
	double run_construct_mtp(std::size_t count, uint32_t frames)
	{
	...
	template <typename T>
	double run_emplace_std(std::size_t count, uint32_t frames)
	{
	...
	template <typename T>
	double run_emplace_pmr(std::size_t count, uint32_t frames)
	{
	...
	template <typename T>
	double run_emplace_mtp(std::size_t count, uint32_t frames)
	{
